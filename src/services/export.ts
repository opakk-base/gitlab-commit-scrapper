...
// Export history item as plain text
export function exportHistoryAsText(item: SummaryHistoryItem, filename: string = "commit-summary") {
  const date = new Date(item.generatedAt).toLocaleString();

  let content = `GitLab Commit Summary
Generated: ${date}
Model: ${item.modelUsed}
Configuration: ${item.configName}
Total Commits Analyzed: ${item.totalCommits}
...
Analysis Info:
- Total Commits: ${item.totalCommits}
- Model Used: ${item.modelUsed}
- Configuration: ${item.configName}
- Generated At: ${date}
`;

  if (item.projectStats) {
    content += `
Project Statistics:
- Unique Contributors: ${item.projectStats.uniqueContributors}
- Unique Projects: ${item.projectStats.uniqueProjects}
- Files With Changes: ${item.projectStats.filesWithChanges}
`;
  }
...
// Export history item as CSV
export function exportHistoryAsCSV(item: SummaryHistoryItem, filename: string = "commit-summary") {
  const date = new Date(item.generatedAt).toLocaleString();
  ...
  const rows = [
    ...
    ["Model", item.modelUsed],
    ["Configuration", item.configName],
    ["Total Commits", String(item.totalCommits)],
    [],
    ["Summary"],
    [stripMarkdown(item.summary)],
  ];
  ...
  if (item.projectStats) {
    rows.push(
      [],
      ["Project Statistics"],
      ["Unique Contributors", String(item.projectStats.uniqueContributors)],
      ["Unique Projects", String(item.projectStats.uniqueProjects)],
      ["Files With Changes", String(item.projectStats.filesWithChanges)]
    );
  }
...
export async function exportHistoryAsPDF(
  ...
  pdf.setFont("helvetica", "normal");
  pdf.setTextColor(100, 100, 100);
  pdf.text(`Generated: ${new Date(item.generatedAt).toLocaleString()}`, margin, currentY);
  currentY += 5;
  pdf.text(`Model: ${item.modelUsed}  |  Config: ${item.configName}  |  Total Commits: ${item.totalCommits}`, margin, currentY);
  currentY += 8;
  ...
  for (const line of lines) {
    ...
    if (line.startsWith("#### ")) {
      ...
      pdf.setFontSize(10);
      pdf.setFont("helvetica", "normal");
      let processedLine = stripMarkdown(line);
      const splitText = pdf.splitTextToSize(processedLine, maxWidth);
      if (splitText.length > 1) {
        checkNewPage(splitText.length * 5 + 2);
      }
...
// Export history item as DOCX
export async function exportHistoryAsDOCX(
  item: SummaryHistoryItem,
  filename: string = "commit-summary"
): Promise<void> {
  const date = new Date(item.generatedAt).toLocaleString();
    ...
    sections: [
      {
        ...
        children: [
            ...
            rows: [
              ...
              new TableRow({
                children: [
                  ...
                  new TableCell({
                    children: [new Paragraph({ children: [new TextRun({ text: item.modelUsed })] })],
                  }),
                ],
              }),
              new TableRow({
                children: [
                  new TableCell({
                    children: [new Paragraph({ children: [new TextRun({ text: "Configuration:", bold: true })] })],
                  }),
                  new TableCell({
                    children: [new Paragraph({ children: [new TextRun({ text: item.configName })] })],
                  }),
                ],
              }),
              new TableRow({
                children: [
                  ...
                  new TableCell({
                    children: [new Paragraph({ children: [new TextRun({ text: String(item.totalCommits) })] })],
                  }),
                ],
              }),
            ],
          ...
          ...parseMarkdownToDocx(item.summary),
